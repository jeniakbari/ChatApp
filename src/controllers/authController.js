...
const registerUser = async (req, res, next) => {
  try {
    ...
    await sendEmail(newUser.email, "Verify your email address", htmlContent);
...
const sendOtp = async (req, res, next) => {
  try {
    ...
    await sendEmail(email, "Your Login OTP", `Your OTP is: ${rawOtp}`);
...
const loginUser = async (req, res, next) => {
  try {
    ...
    if (!user)
      return res.status(404).json({ message: "You are not registered user" });
    if (user.failed_attempts === 3) {
      return res
        .status(403)
        .json({
          message:
            "Your account is locked due to multiple failed attempts. Please try again tomorrow",
        });
    }
