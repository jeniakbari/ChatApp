...
import { demoChatAPI, demoUserAPI } from '../services/demoApi';
import { useDemoSocket } from '../context/DemoSocketContext';
...
const DemoDashboard = () => {
  ...
  const { isUserOnline } = useDemoSocket();
  ...
  const fetchChats = async () => {
    try {
      const response = await demoChatAPI.getChats();
  ...
  const searchUsers = async () => {
    ...
    try {
      const response = await demoUserAPI.searchUsers(searchQuery);
  ...
  const handleUserClick = (selectedUser) => {
    ...
    if (existingChat) {
      navigate(`/demo-chat/${existingChat.id}`);
    } else {
      // Navigate to chat with user info to create new chat
      navigate(`/demo-chat/new`, { state: { user: selectedUser } });
    }
    setSearchQuery('');
    setSearchResults([]);
  };
  ...
  const formatLastMessage = (message) => {
    if (!message || !user) return 'No messages yet';
  ...
  // Show loading spinner while auth is loading or user is not available
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-400">Welcome back, {user.username}! Stay connected with your friends</p>
            </div>
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <Avatar
                            ...
                            alt={chat.name || otherParticipant?.username || 'Unknown'}
                            size="md"
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between">
                              <h3 className="font-medium text-white truncate">
                                {chat.name || otherParticipant?.username || 'Unknown Chat'}
                              </h3>
                              {chat.lastMessage && (
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        <Avatar
                          ...
                          size="sm"
                          online={isUserOnline(searchUser.id)}
                        />
                        <div>
                          <h4 className="font-medium text-white text-sm">{searchUser.username}</h4>
